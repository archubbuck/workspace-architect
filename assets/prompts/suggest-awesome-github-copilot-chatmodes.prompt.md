...
2. **Scan Local Custom Chat Modes**: Discover existing custom chat mode files in `assets/agents/` folder
...
10. **Download Assets**: For requested chat modes, automatically download and install individual chat modes to `assets/agents/` folder. Do NOT adjust content of the files. Use `#todos` tool to track progress. Prioritize use of `#fetch` tool to download assets, but may use `curl` using `#runInTerminal` tool to ensure all content is retrieved.
...
1. List all `*.agent.md` files in `assets/agents/` directory
...
- Scan local file system for existing chatmodes in `assets/agents/` directory
